	...
	const fetchData = async () => {
		...
		try {
			...
			if (window.localStorage.getItem("type") === "trainlink"){
				contract.contract =  await window?.tronWeb?.contract().at('TKAQLc3C8u9XcTQeroVtzBZrRpHJXQeUaK');
				...
				contract.contract = await tronWeb.contract().at('TKAQLc3C8u9XcTQeroVtzBZrRpHJXQeUaK');
				window.contract = contract.contract;
				setContractInstance(contract);
			}
		} catch (error) {
