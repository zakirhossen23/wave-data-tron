	...
	const fetchData = async () => {
		...
		try {
			...
			if (window.localStorage.getItem("type") === "trainlink"){
				contract.contract =  await window?.tronWeb?.contract().at('TSSo8Mo4dg8PS8krQtAtjdsjetVTX7xNVL');
				...
				contract.contract = await tronWeb.contract().at('TSSo8Mo4dg8PS8krQtAtjdsjetVTX7xNVL');
				window.contract = contract.contract;
				setContractInstance(contract);
			}
		} catch (error) {
