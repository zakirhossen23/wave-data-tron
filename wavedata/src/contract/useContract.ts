	...
	const fetchData = async () => {
		...
		try {
			...
			if (window.localStorage.getItem("type") === "trainlink"){
				contract.contract =  await window?.tronWeb?.contract().at('TLyymjmJSzWw4PHoM81wG7gN2yNHytAUJ4');
				...
				contract.contract = await tronWeb.contract().at('TLyymjmJSzWw4PHoM81wG7gN2yNHytAUJ4');
				window.contract = contract.contract;
				setContractInstance(contract);
			}
		} catch (error) {
