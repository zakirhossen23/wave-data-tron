	...
	const fetchData = async () => {
		...
		try {
			...
			if (window.localStorage.getItem("type") === "trainlink"){
				contract.contract =  await window?.tronWeb?.contract().at('TJBfdgXx18L4ZzEqciKr9ysXS8pnrumSc6');
				...
				contract.contract = await tronWeb.contract().at('TJBfdgXx18L4ZzEqciKr9ysXS8pnrumSc6');
				window.contract = contract.contract;
				setContractInstance(contract);
			}
		} catch (error) {
