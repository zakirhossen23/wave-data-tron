	...
	const fetchData = async () => {
		...
		try {
			...
			if (window.localStorage.getItem("type") === "trainlink"){
				contract.contract =  await window?.tronWeb?.contract().at('TTYE69G3GARC3ScSXZe1ie4xK9eAmQLdpB');
				...
				contract.contract = await tronWeb.contract().at('TTYE69G3GARC3ScSXZe1ie4xK9eAmQLdpB');
				window.contract = contract.contract;
				setContractInstance(contract);
			}
		} catch (error) {
